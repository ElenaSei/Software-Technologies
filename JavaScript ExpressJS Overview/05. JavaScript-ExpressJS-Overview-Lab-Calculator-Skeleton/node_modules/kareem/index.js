    ...
    var next = function () {
        ...
        if (firstError) {
            ...
            if (post.length === numArgs + 1) {
                ...
                if (isPromise(maybePromise)) {
                    return maybePromise.then(() = > _cb(), err =
                >
                    _cb(err)
                )
                    ;
...
Kareem.prototype.merge = function (other) {
    ...
    for (let key of other._pres.keys()) {
        ...
        filter(p = > sourcePres.map(_p = > _p.fn
    ).
        indexOf(p.fn) === -1
    )
        ;
        ...
        combined.numAsync += deduplicated.filter(p = > p.isAsync
    ).
        length;
