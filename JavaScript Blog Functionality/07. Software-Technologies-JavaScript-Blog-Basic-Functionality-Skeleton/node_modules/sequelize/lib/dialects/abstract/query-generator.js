...
const QueryGenerator = {
    ...
    */
    bulkInsertQuery(tableName, fieldValueHashes, options, fieldMappedAttributes) {
        ...
        const replacements = {
            ...
            attributes: allAttributes.map(attr = > this.quoteIdentifier(attr)
    ).
        join(','),
            tuples
    :
        tuples.join(','),
            ...
            returning
    :
        this._dialect.supports.returnValues && options.returning ? ' RETURNING *' : ''
    }
        ;
    ...
    */
    selectQuery(tableName, options, model) {
        ...
        if (options.group) {
            options.group = Array.isArray(options.group) ? options.group.map(t = > this.quote(t, model)
        ).
            join(', ')
        :
    ...
    whereItemsQuery(where, options, binding) {
        ...
        return items.length && items.filter(item = > item && item.length
    ).
        join(binding) || '';
    },
    ...
    _getSafeKey(key, prefix) {
        ...
        if (Utils.isColString(key)) {
            ...
            return key.map(identifier = > this.quoteIdentifier(identifier)
        ).
            join('.');
        }
