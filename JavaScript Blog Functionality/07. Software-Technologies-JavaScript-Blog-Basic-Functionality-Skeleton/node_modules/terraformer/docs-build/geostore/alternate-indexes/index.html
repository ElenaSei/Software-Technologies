...
<main class="docs main-content" id="main-content">
    ...
    <p>Alternate Indexes are a very powerful way to apply additional filters to your searches on <code>properties</code>
        of a <code>Feature</code>.</p>
    <h2>
...
<span class="nx">gs</span><span class="p">.</span><span class="nx">addIndex</span><span class="p">({</span>
  ...
  <span class="na">index</span><span class="p">:</span> <span class="k">new</span> <span class="nx">BTree</span><span
                class="p">()</span>
<span class="p">});</span>
...
<span class="p">});</span>
    ...
    <p>Queries against Alternate Indexes are very specific to the index, but the basic syntax of a query is
        straightforward:</p>
    <pre class="highlight javascript"><code><span class="nx">gs</span><span class="p">.</span><span
            class="nx">within</span><span class="p">(</span>
...
<span class="p">);</span>
    ...
    <p>In this example, first every <code>Feature</code> that is within the GeoJSON is found in the Spatial Index, then
        each of those entries are searched for in the B-Tree indexes. Only those entries that have a <code>properties.street</code>
        of &ldquo;Main&rdquo; and a <code>properties.crime</code> of &ldquo;Arson&rdquo; are returned.</p>
    <h3>
    ...
    <p>Only data that is available in <code>properties</code> is indexable. When a <code>Feature</code> is added to the
        GeoStore, the list of Alternate Indexes is scanned, and any property that is found is added to that Alternate
        Index.</p>
    ...
    <p>Alternate Indexes rely on two methods to be exposed to the GeoStore to add and remove data. Additional methods
        can be added for querying and are introspected for searching.</p>

    <p>For instance, when a <code>equals</code> query is made against an Alternate Index, if the <code>equals</code>
        method is on the index, then it is called with all arguments from the <code>contains</code> or
        <code>within</code>.</p>
    <h3>
    ...
    <p><em>Example:</em></p>
    <pre class="highlight javascript"><code><span class="nx">idx</span><span class="p">.</span><span
            class="nx">add</span><span class="p">(</span><span class="nx">value</span><span class="p">,</span> <span
            class="nx">id</span><span class="p">,</span> <span class="kd">function</span> <span class="p">(</span><span
            class="nx">err</span><span class="p">,</span> <span class="nx">res</span><span class="p">)</span> <span
            class="p">{</span>
  <span class="c1">// Node.js style callback</span>
<span class="p">});</span>
</code></pre>
