...
<main class="docs main-content" id="main-content">
    ...
    <div class='semantic-toc'>
        <ol>
            <li><a href='#existing-data-stores'>Existing Data Stores</a></li>
            <li><a href='#writing-a-datastore'>Writing a DataStore</a>
                <ol>
                    <li><a href='#methods'>Methods</a>
                        <ol>
                            <li><a href='#new-datastore'>new DataStore()</a></li>
                            <li><a href='#datastoreadd'>DataStore.add(geostore, callback)</a></li>
                            <li><a href='#datastoreupdate'>DataStore.update(geostore, callback)</a></li>
                            <li><a href='#datastoreremove'>DataStore.remove(id, callback)</a></li>
                            <li><a href='#datastoreget'>DataStore.get(id, callback)</a>
    </div>

    <p>Data stores are the foundation of the GeoStore. They are <code>key/value</code> storage devices that allow for
        creating entries, updating, deleting, and retrieving single <code>Feature</code> objects in GeoJSON.</p>

    <p>Data Stores are designed to be asyncronous, using Node.js style callbacks. In syncronough stores, like <code>Terraformer.Store.Memory</code>
        and <code>Terraformer.Store.LocalStorage</code> callbacks are executed immediately, but in truly asyncronous
        stores they behave as expected.</p>
    <h2>
    ...
    <ul>
        <li><a href="https://github.com/Esri/terraformer-geostore-localstorage">LocalStorage</a> - works in the browser
            only
        </li>
        <li><a href="https://github.com/Esri/terraformer-geostore-memory">Memory</a> - works in both the browser and
            Node.js
        </li>
    ...
    <p>Since Data Stores are simply <code>key/value</code> stores, it is very easy to write additional Data Stores as
        long as the method signatures are correct.</p>
    <h3>
    ...
    <p>Instantiating a <code>DataStore</code> should return a new object containing method signatures conforming to the
        <code>DataStore</code> interface.</p>

    <p>You can pass any needed arguments while instantiating.</p>

    <p><em>Example:</em></p>
    <pre class="highlight javascript"><code><span class="kd">var</span> <span class="nx">ds</span> <span
            class="o">=</span> <span class="k">new</span> <span class="nx">DataStore</span><span class="p">({</span>
...
<span class="p">});</span>
    ...
    <p>Add a <code>geojson</code> object to a DataStore. In the case of a <code>Feature</code>, the <code>id</code>
        should be used as the primary key for storage and retrieval:</p>
    <pre class="highlight javascript"><code><span class="c1">// get the id</span>
<span class="kd">var</span> <span class="nx">id</span> <span class="o">=</span> <span class="nx">geojson</span><span
                class="p">.</span><span class="nx">id</span><span class="p">;</span>

<span class="c1">// store the data</span>
<span class="k">this</span><span class="p">.</span><span class="nx">magicdb</span><span class="p">.</span><span
                class="nx">put</span><span class="p">(</span><span class="nx">geojson</span><span
                class="p">.</span><span class="nx">id</span><span class="p">,</span> <span class="nx">JSON</span><span
                class="p">.</span><span class="nx">stringify</span><span class="p">(</span><span
                class="nx">geojson</span><span class="p">));</span>
</code></pre>

    <p>If a <code>FeatureCollection</code> is passed in instead, each <code>Feature</code> inside of the <code>FeatureCollection</code>
        needs to be added before the <code>callback</code> is called.</p>
    ...
    <p><em>Example:</em></p>
    <pre class="highlight javascript"><code><span class="nx">ds</span><span class="p">.</span><span
...
<span class="p">});</span>
    ...
    <p>Update a <code>geojson</code> object already in a DataStore. Only a <code>Feature</code> should be able to be
        updated, the <code>id</code> should be used as the primary key for update:</p>
